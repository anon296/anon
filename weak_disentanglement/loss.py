...
class DiscriminatorLossCompute(nn.Module):
    ...
    def __call__(self, z_gen_score, z_prior_score, z_gen, z_prior, retain=False):
        ...
        loss_gen   = self.criterion(z_gen_score, torch.zeros_like(z_gen_score).to(self.device))
        loss_prior = self.criterion(z_prior_score, torch.ones_like(z_prior_score).to(self.device))
...
class GeneratorLossCompute(nn.Module):
    ...
    def __call__(self, z_gen_score, retain=False):
        ...
        loss = self.criterion(z_gen_score, torch.ones_like(z_gen_score).to(self.device))
        loss = self.cfg.loss.adversarial_weight * loss.mean()
