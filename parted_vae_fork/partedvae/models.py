...
class PartialLogSoftmax:
    def __init__(self, dimensions):
        ...
        self.minus_inf = torch.tensor([float('-inf')], requires_grad=False).cuda()
        self.eps = torch.tensor([EPS], requires_grad=False).cuda()

        self.unwrap_logits_mask = torch.zeros(1, self.sum_dims, self.sum_dims, len(dimensions), requires_grad=False).cuda()
        self.scatter_log_denominator_mask = torch.zeros(len(dimensions), self.sum_dims, requires_grad=False).cuda()
...
class VAE(nn.Module):
    def __init__(self, img_size, latent_spec, c_priors, temperature=0.67):
        ...
        if self.has_dep:
            self.c_priors = torch.tensor(sum(c_priors, []), requires_grad=False).cuda()
        ...
        self.features_to_hidden = nn.Sequential(
            nn.Linear(np.prod(self.reshape), self.hidden_dim).cuda(),
            nn.LeakyReLU(0.1),
        )
        ...
        if self.has_indep:
            self.z_mean_fc = nn.Linear(self.hidden_dim, self.z_dim).cuda()
            self.z_logvar_fc = nn.Linear(self.hidden_dim, self.z_dim).cuda()

        if self.has_dep:
            self.h_to_c_logit_fc = nn.Linear(self.hidden_dim, self.sum_c_dims).cuda()

            self.c_to_a_logit_pc = nn.Linear(self.sum_c_dims, self.c_count * self.hidden_dim).cuda()  # A Sigmoid should be placed after this layer
            self.c_to_a_logit_mask = torch.zeros(self.c_count * self.hidden_dim, self.sum_c_dims, requires_grad=False).cuda()
            h_start = 0
            ...
            self.h_dot_a_to_u_mean_pc = nn.Linear(self.c_count * self.hidden_dim, self.u_dim).cuda()
            self.h_dot_a_to_u_logvar_pc = nn.Linear(self.c_count * self.hidden_dim, self.u_dim).cuda()
            self.h_dot_a_to_u_mask = torch.zeros(self.u_dim, self.c_count * self.hidden_dim, requires_grad=False).cuda()
            ...
            self.c_to_a_logit_mask = self.c_to_a_logit_mask.cuda()
            self.h_dot_a_to_u_mask = self.h_dot_a_to_u_mask.cuda()

        # Decoder
        self.latent_to_features = nn.Sequential(
            nn.Linear(self.latent_dim, self.hidden_dim).cuda(),
            nn.ReLU(),
            nn.Linear(self.hidden_dim, np.prod(self.reshape)).cuda(),
            nn.ReLU()
        )
        ...
        self.logsoftmaxer = PartialLogSoftmax(self.c_dims)
