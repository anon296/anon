...
class Trainer:
    def __init__(self, model, optimizers, test, dataset=None, recon_type=None, z_capacity=None,
        ...
        self.u_kl_func_valid_indices = torch.zeros(self.model.sum_c_dims, dtype=torch.long, requires_grad=False).cuda()
        start = 0
        ...
        self.unwrap_mask = torch.zeros(self.model.sum_c_dims, self.model.sum_c_dims, self.model.c_count, requires_grad=False).cuda()
        start = 0
        ...
        self.u_valid_prior_BC_mask = torch.zeros(self.model.sum_c_dims, self.model.sum_c_dims, requires_grad=False).cuda()
        start = 0
        for dim_idx, size in enumerate(self.model.c_dims):
    ...
    def train(self, data_loader, warmuploader=None, epochs=10, run_after_epoch=None, run_after_epoch_args=None):
        self.batch_size = data_loader.batch_size
        self.model.train()

        if warmuploader is None:
            print('No warm-up')

        for epoch in range(epochs):
            t = time()
            warm_up_mean_loss, separated_mean_epoch_loss = self._train_epoch(data_loader, warmuploader)
            others, kl_z_dims = [
            ...
            if self.test: break
            print(f'Epoch: {epoch}\tMloss: {others[0]}\tWarmUpLoss: {warm_up_mean_loss}\tTime: {round((time() - t) / 60)}')

            if warmuploader is not None:
    ...
    def _train_epoch(self, data_loader, warmuploader=None):
        warm_up_loss = self._warm_up(warmuploader)
        separated_sum_loss = 0
    ...
    def _warm_up(self, loader):
        ...
        for data, label in loader:
            ...
            loss = F.cross_entropy(latent_dist['log_c'],label[:,4])
    ...
    def _loss_function(self, data, recon_data, latent_dist):
        ...
        tmp_zero = torch.zeros(1, requires_grad=False).cuda()
