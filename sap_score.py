    ...
    for i in range(num_latents):
        for j in range(num_factors):
            ...
            if continuous_factors:
                ...
                y_j_test = ys_test[j, :]
                classifier = svm.LinearSVC(C=0.01, class_weight="balanced",max_iter=250,dual=False)
...
def compute_avg_diff_top_two(matrix):
    ...
    per_factor = sorted_matrix[-1, :] - sorted_matrix[-2, :]
    return {'per_factor': per_factor.tolist(), 'avg': np.mean(per_factor)}
...
if __name__ == '__main__':
    ...
    c = _compute_sap(mutr,ytr,muts,yts,continuous_factors=None)
    print(c)
