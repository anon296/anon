...
def _reconstruction_loss(data, recon_data, distribution="bernoulli", storer=None):
    ...
    if (recon_data>1).any() or (recon_data<0).any():
        import pdb; pdb.set_trace()  # XXX BREAKPOINT

    #try:
        #_ = F.binary_cross_entropy(recon_data.detach().cpu(), data.detach().cpu(), reduction="sum")
    #except:
        #import pdb; pdb.set_trace()  # XXX BREAKPOINT
...
def _kl_normal_loss(mean, logvar, storer=None):
    ...
    latent_kl_ = -1 - logvar + mean.pow(2) + logvar.exp()
    #inf_mask = torch.isinf(latent_kl_).any(axis=1)
    clip_mask = (latent_kl_>1e+5).any(axis=1)
    if clip_mask.any():
        print(f'excluding {clip_mask.sum().item()} rows because too big')
    latent_kl = 0.5 * latent_kl_[~clip_mask].mean(dim=0)
...
def _permute_dims(latent_sample):
    ...
    np_perm = numpyify(perm)
    batch_size, dim_z = perm.size()

    for z in range(dim_z):
        pi = torch.randperm(batch_size).to(latent_sample.device)
        try:
            numpyify(latent_sample)[numpyify(pi), numpyify(z)]
        except:
            import pdb; pdb.set_trace()  # XXX BREAKPOINT
