from .sagan import Generator, Discriminator, Discriminator_MLP
from .resnet import resnet50, resnet18
from disentangling_vae_fork.disvae.models.encoders import EncoderBurgess
from disentangling_vae_fork.disvae.models.decoders import DecoderBurgess
import numpy as np
...
class BGM(nn.Module):
    ...
    '''
    def __init__(self, latent_dim=64, conv_dim=32, nc=1,
        ...
        self.encoder = EncoderBurgess(img_size=(nc,64,64), latent_dim=latent_dim)
        #self.decoder = BigDecoder(latent_dim, conv_dim, image_size, dec_dist)
        self.decoder = DecoderBurgess(img_size=(nc,64,64), latent_dim=latent_dim)
        if 'scm' in prior:
...
class BigJointDiscriminator(nn.Module):
    ...
    def __init__(self, latent_dim=64, conv_dim=32, image_size=64, fc_size=1024):
        ...
        self.discriminator_z = Discriminator_MLP(latent_dim, fc_size, num_block=2)
        self.discriminator_j = Discriminator_MLP(conv_dim * 4 + fc_size, fc_size, num_block=2)
    ...
    def forward(self, x, z):
        ...
        return (sx + sz.squeeze(1) + sxz.squeeze(1)) / 3
